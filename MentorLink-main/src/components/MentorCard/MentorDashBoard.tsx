...
export default function MentorDashboard() {
  ...
  const fetchNotifications = async (currentUserId: string): Promise<void> => {
    ...
    try {
      ...
      if (mentorSubjects && mentorSubjects.length > 0) {
        const subjectNames = mentorSubjects.map((s: any) => s.course_name).filter(Boolean);
        ...
        if (questionsData && questionsData.length > 0) {
          ...
          const existingQids = new Set(existingNotifs?.map((n: any) => n.question_id) || []);

          // Find questions without notifications
          const missingNotifs = questionsData
            .filter((q: any) => !existingQids.has(q.question_id))
            .map((q: any) => ({
      ...
      const questionIds = notifData.map((n: any) => n.question_id).filter((id: any) => id != null);
      ...
      const repliedSet = new Set(replyData?.map((r: any) => r.question_id) || []);
  ...
  const fetchMentorRank = async (currentUserId: string): Promise<void> => {
    ...
    try {
      ...
      const mentorIds = (globalMentors || []).map((m: any) => m.mentor_id);
      if (mentorIds.length > 0) {
        ...
        allReplies?.forEach((r: any) => { if(r.mentor_id) rMap[r.mentor_id] = (rMap[r.mentor_id] || 0) + 1; });
        ...
        const scoreboard = mentorIds.map((id: any) => {
        ...
        const derivedRank = scoreboard.findIndex((item: any) => item.id === currentUserId) + 1;
    ...
    try {
      const chatCompletion = await groq.chat.completions.create({
        messages: [
          {
            role: "system",
            content:"You are an expert university-level exam question generator. Your task is to create exactly 15 multiple-choice questions (MCQs) ,no more no less, exxactly 15, in json format based on the subject provided by the user. Each question should have normal difficulty, suitable for university students. You MUST output ONLY valid JSON using this exact schema, with no additional text or formatting: { \"questions\": [ { \"question\": \"...\", \"options\": [\"A\", \"B\", \"C\", \"D\"], \"correctAnswer\": 0 } ] } where correctAnswer is the integer index (0-3) of the correct option. Ensure options are distinct. and make sure there are 15 questions, no more no less, also make sure to change the content everytime you are asked to send test questions, change the topics of the questions or rephrase them as compared to last test and remember to keep the correct options random, dont prioritize a,b,c or d option, just do random, and make sure they dont have a distinct pattern like 6 consecutive correct a's"
          },
